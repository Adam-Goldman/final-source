...
// RAVEN END
	...
	lastAttack		= 0;
 	clipPredictTime	= 0;
	//my code
	int rof = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100));;
	int ac = rand() % 100;
	//my code
	...
	fireRate	= rof;//SEC2MS ( spawnArgs.GetFloat ( "fireRate" ) );
	...
	// Ammo
	clipSize			= ac;//spawnArgs.GetInt( "clipSize" );
...
void rvWeapon::InitViewModel( void ) {
	...
	if ( spawnArgs.GetString ( "skin", "", &temp ) ) {
		...
		// DJ see about overrideing model shown here
	}
...
void rvWeapon::Attack( bool altAttack, int num_attacks, float spread, float fuseOffset, float power ) {
	...
	idMat3 muzzleAxis;
	//Adam's Code
	float newPower = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100));
	float boom = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100));
	...
	if ( !gameLocal.isClient ) {
		...
		//randomly choosing power
		if (altAttack ? wfl.attackAltHitscan : wfl.attackHitscan) {
			Hitscan( dict, muzzleOrigin, muzzleAxis, num_attacks, spread, newPower );
		} else {
			LaunchProjectiles( dict, muzzleOrigin, muzzleAxis, num_attacks, spread, fuseOffset, newPower );
		}
